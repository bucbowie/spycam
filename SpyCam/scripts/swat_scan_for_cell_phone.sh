...
#-----------------------------------------------------------#
export mySCRIPT_DIR=/home/swat/scripts
cd /home/swat/scripts
set -avx
...
#-----------------------------------------------------------#
 ...
 if [[ -f ./swat.config ]]; then
 ...
 else
   {
      ...
      echo "# Aborting scripts with no action taken.  "
      exit -1
   }
 fi
 ...
 COUNT_HIT_ON_CELL_PHONE=`hcitool -i hci0 info "${CELL_BLUETOOTH_ADDRESS}"|grep -w Features|wc -l`
 if [[ $((COUNT_HIT_ON_CELL_PHONE)) -eq  0 ]];then
   {
	 #-------------------------------------------------------#
	 ...
	 if [[ "${SCAN_ROUTER}" == \""${MY_ROUTER_NAME}"\" ]]; then
	   {
	      ...
	      COUNT_HIT_ON_CELL_PHONE=`arp-scan --interface=wlan0 "${MY_ROUTER_IP}/24" 2>/dev/null|grep "${CELL_STATIC_IP}"|wc -l`
	   }
	 else
	   {
	   #-----------------------------------------------------#
	   # Router not available. Do not trip the COUNT_HIT_ON_CELL_PHONE=0.
	   #-----------------------------------------------------#
	     let COUNT_HIT_ON_CELL_PHONE=1
	   }
	 fi
   }
 fi
 echo "COUNT_HIT_ON_CELL_PHONE=${COUNT_HIT_ON_CELL_PHONE}"
 if [[  $((COUNT_HIT_ON_CELL_PHONE)) -eq 0  ]];then
   {
...
#      fi
#  }
 if [[ $((COUNT_HIT_ON_CELL_PHONE)) -gt 0 ]];then
   {
